...
export const handler = async (event: EventType) => {
    try {
        ...
        if(!process.env.COLLECTION_QUEUE_URL){
            throw(new Error('Missing COLLECTION_QUEUE_URL environment variable'));
        }
        if(!process.env.COLLECTION_OS_QUEUE_NAME){
            throw(new Error('Missing COLLECTION_OS_QUEUE_NAME environment variable'));
        }
        if(!process.env.NFT_QUEUE_NAME){
            throw(new Error('Missing NFT_QUEUE_NAME environment variable'));
        }
        ...
        if(event.Records){
            for (const record of event.Records){
                ...
                await processItem(collectionId);
            }
