...
export const handler = async (event: EventType) => {
    try {
        ...
        */
        ...
        let counter = 0;
        const mintAddresses: AWS.SQS.Message[] = [];
        ...
        while(counter < MAX_SQS_BATCHES){
            ...
            if(tmpArray.length > 0){
                console.log(`Adding ${tmpArray.length} items to mintAddresses list`)
                mintAddresses.push.apply(mintAddresses, tmpArray);
        ...
        /** Unique-ify items */
        const uniqueMintAddresses = [...new Set(mintAddresses.map(item => item.Body).values())];
        console.log(`mintAddresses received: Total=${mintAddresses.length} | Unique=${uniqueMintAddresses.length}`);

        /** Send unique collection ids to outbound SQS queue */
        await sendToQueue(sqs, outboundQueueUrl, uniqueMintAddresses);
        //If we have some items process each one of them and delete it from the queue
        if(mintAddresses){
            console.log(`Processing ${mintAddresses.length} mintAddresses`);
