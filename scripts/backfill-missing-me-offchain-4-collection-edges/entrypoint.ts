...
(async () => 
{
    ...
    let i = 1;
    let batchNumber = 1;
    const mintToCollection = new Map<string,string>();
    for await (const data of rl) {
        const [mint, collectionId] = data.split(" ");
        mintToCollection.set(mint, collectionId);
    }
    ...
    for await (const item of ddbmapper.query( 
        ...
        const collectionId = mintToCollection.get(item.primaryEntity);
        if (!collectionId) {
            continue;
        }
        const md = new OffchainMetadata4CollectionEntity();
        md.populate({ globalId: item.globalId, type: EntityType.MetaplexOffchainMetadata }, collectionId);
