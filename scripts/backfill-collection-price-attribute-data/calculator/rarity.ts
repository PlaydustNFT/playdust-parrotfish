...
export function calculateStatisticalRarityForMint(
  ...
  if (attributes) {
    for (const attribute of attributes) {
      if (!attributeMap.has(attribute.trait_type)) {
        continue;
      }
      let mult = attributeMap.get(attribute.trait_type).get(attribute.value)?.count
              ? (attributeMap.get(attribute.trait_type).get(attribute.value).count / totalCount)
              : 1;
      rarity *= mult;
    }
  }
