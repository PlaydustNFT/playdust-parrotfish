...
const LARGE_COLLECTIONS_FILE =
        process.env.LARGE_COLLECTIONS_FILE
        || "/home/ec2-user/files/large_collections";
const MAX_ITEM_LENGTH_DDB =
        process.env.MAX_ITEM_LENGTH_DDB
        || 400000;
// for now, let's leave out sqs in backfill collection processor (run it separate)
...
export const batchPutRarityData = async (rarityData: NFTRarityData[], collectionId: string) => {
    ...
    rarityData = [...new Map(rarityData.map(item => [item.mint, item])).values()];
...
export const processItem = async (collectionId: string) => {
    ...
    {
        //console.log(`related_item=${JSON.stringify(md.data)}`);
    ...
    if (relatedOffchainMetadataFromQuery !== undefined && relatedOffchainMetadataFromQuery.length > MAX_COLLECTION_SIZE) {
        console.log(`LARGE_COLLECTION|collectionId=${collectionId}|size=${relatedOffchainMetadataFromQuery.length}`);
        writeToFile([collectionId], 1, LARGE_COLLECTIONS_FILE);
        return;
    }
    ...
    if (isOversizedForDynamoDB(collectionAttributeData)) {
        let l = Buffer.from(JSON.stringify(collectionAttributeData)).length;
        console.log(`FAIL|reason=too big|length=${JSON.stringify(l)}|data=${JSON.stringify(collectionAttributeData)}`);
    }
    else {
    ...
    /** Write mint address list to the SQS queue */
    //sendToQueue(sqs, queueUrl, mintAddressList); // comment out for now
    ...
    if(collectionData){
        ...
        if (isOversizedForDynamoDB(collectionMetadataEntity)) {
            let l = Buffer.from(JSON.stringify(collectionMetadataEntity)).length;
            console.log(`FAIL|reason=too big|length=${l}|data=${JSON.stringify(collectionAttributeData)}`);
        }
        else{
